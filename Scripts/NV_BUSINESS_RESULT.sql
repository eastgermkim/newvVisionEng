...
/*사업실적 이미지 테이블*/
CREATE TABLE NV_BUSINESS_RESULT_IMG(
   RESULT_IMG_NUM BIGINT AUTO_INCREMENT PRIMARY KEY,
   RESULTNUM BIGINT UNIQUE,
   /*BASIC_OR_CUSTOM INT NOT NULL,*/ 			/*기본이미지 사용시 0, 다른 이미지 첨부시 1*/
   /*BASIC_SRC INT NOT NULL, */					/*기본이미지 사용시 이미지 주소*/
   /*CUSTOM_SYSTEMNAME VARCHAR(255) UNIQUE,*/ 	/*다른 이미지 첨부시 - 저장되는 파일명*/
   /*CUSTOM_ORGNAME VARCHAR(1000),*/			/*다른 이미지 첨부시 - 원래 파일명*/
   CONSTRAINT NV_BUSINESS_RESULT_IMG_FK FOREIGN KEY(RESULTNUM) 
   REFERENCES NV_BUSINESS_RESULT(RESULTNUM) ON DELETE CASCADE
);
	
SELECT * FROM NV_BUSINESS_RESULT_IMG;
INSERT INTO NV_BUSINESS_RESULT_IMG(RESULTNUM,BASIC_OR_CUSTOM) VALUES(2,0);
...
DROP TABLE NV_BUSINESS_RESULT_IMG;
